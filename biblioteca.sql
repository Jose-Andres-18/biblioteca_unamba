...
INSERT INTO `estudiante` (`id`, `codigo`, `dni`, `nombre`, `carrera`,
 `direccion`, `telefono`, `estado`) VALUES
...
INSERT INTO `libro` (`id`, `titulo`, `cantidad`, `id_autor`, `id_editorial`,
 `anio_edicion`, `id_materia`, `num_pagina`, `descripcion`, `imagen`, `estado`) VALUES
...
-- Agregar la columna id_carrera a la tabla estudiante y establecer la relación
ALTER TABLE estudiante
  ADD COLUMN id_carrera int(11) NOT NULL;

-- Actualizar los valores iniciales de id_carrera en estudiante
UPDATE estudiante SET id_carrera = 1 WHERE id = 1; -- Ejemplo para asociar un estudiante con una carrera
UPDATE estudiante SET id_carrera = 2 WHERE id = 2;

-- Establecer la clave foránea entre estudiante y carrera
ALTER TABLE estudiante
  ADD CONSTRAINT estudiante_ibfk_1 FOREIGN KEY (id_carrera) REFERENCES carrera (id);

-- Agregar índice a la columna id_carrera
ALTER TABLE estudiante
  ADD KEY id_carrera (id_carrera);

-- Establecer la relación entre libro y materia
ALTER TABLE libro
  ADD CONSTRAINT libro_ibfk_4 FOREIGN KEY (id_materia) REFERENCES materia (id);

-- Agregar índice a la columna id_materia en libro
ALTER TABLE libro
  ADD KEY id_materia (id_materia);

-- Establecer la relación entre prestamo y usuario
ALTER TABLE prestamo
  ADD COLUMN id_usuario int(11) NOT NULL;

-- Actualizar valores iniciales de id_usuario en prestamo
UPDATE prestamo SET id_usuario = 1 WHERE id = 1; -- Asociar préstamos con usuarios
UPDATE prestamo SET id_usuario = 2 WHERE id = 2;

-- Establecer la clave foránea entre prestamo y usuarios
ALTER TABLE prestamo
  ADD CONSTRAINT prestamo_ibfk_3 FOREIGN KEY (id_usuario) REFERENCES usuarios (id);

-- Agregar índice a la columna id_usuario en prestamo
ALTER TABLE prestamo
  ADD KEY id_usuario (id_usuario);
